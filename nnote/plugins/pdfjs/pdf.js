  ...
  exports.FontLoader = FontLoader = class GenericFontLoader extends BaseFontLoader {
    ...
    _prepareFontLoadEvent(rules, fonts, request) {
      ...
      function isFontReady(name, callback) {
        ...
        ctx.font = "30px " + name;
        if(!window.pdf2html || !window.pdf2html.canvasOnlyRenderSpecialChar()){
            ctx.fillText(".", 0, 20);
        }
    ...
    for (i = 0; i < glyphsLength; ++i) {
      ...
      if (this.contentVisible && (glyph.isInFont || font.missingFile)) {
        if (simpleFillText && !accent) {
          //var _drawcode="+-*/=<>−¯(){}[]|"
          //console.log("text "+glyph.unicode+":"+glyph.unicode.charCodeAt(0));
          //if(glyph.unicode.charCodeAt(0)>7930 || _drawcode.indexOf(glyph.unicode)>=0){
            
          //}
          if(!window.pdf2html || !window.pdf2html.canvasOnlyRenderSpecialChar() || window.pdf2html.isSpecialChar(glyph.unicode)){
...
exports.SVGGraphics = SVGGraphics;
{
  ...
  exports.SVGGraphics = SVGGraphics = class {
      ...
      for (const opTreeElement of opTree) {
        ...
        switch (fnId | 0) {
          ...
          case _util.OPS.showText:
           // console.log("showText : "+JSON.stringify(args[0]));
            this.showText(args[0]);
            break;

          case _util.OPS.showSpacedText:
           // console.log("showSpacedText : "+JSON.stringify(args[0]));
            this.showText(args[0]);
            break;
    ...
    showText(glyphs) {
     ...
     // console.log("word:char :"+wordSpacing+":"+charSpacing);

      //if(!current.tspan.carr) current.carr="";
      var carr=current.tspan.getAttribute("carr");
      if(!carr) carr="";

      var carrs=current.tspan.getAttribute("carrs");
      if(!carrs) carrs="";
      
      var warr=current.tspan.getAttribute("warr");
      if(!warr) warr="";  
      ...
      for (const glyph of glyphs) {
        if (glyph === null) {
          ...
          //console.log("glyph number "+glyph+":"+fontSize+":"+charSpacing);
          x += spacingDir * glyph * fontSize / 1000;
          continue;
        }
        ...
        if (glyph.isInFont || font.missingFile){
           ...
           warr+=Number(width * widthAdvanceScale/fontSizeScale)+" ";
           carr+=glyph.unicode;
      ...
      current.tspan.setAttributeNS(null, "warr",warr); 

      current.tspan.setAttributeNS(null, "carr",carr); 

      current.tspan.setAttributeNS(null, "carrs",carrs);
    ...
    _makeTilingPattern(args) {
      ...
      if(bbox.childNodes.length>0) tiling.appendChild(bbox.childNodes[0]);
