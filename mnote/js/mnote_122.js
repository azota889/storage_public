...
var MNote_instance = cc.Class.extend({
        ...
        exportJSON: function() {
            ...
            $(b).each(function(d) {
                ...
                k = $(k).find(".obj_text");
                d.objText = [];
                for (g = 0; g < k.length; g++) l = {}, l.x = parseInt($(k[g]).css("left")), l.y = parseInt($(k[g]).css("top")), l.value = k[g].textContent, l.textStyle = JSON.parse($(k[g]).attr("styleText").replaceAll("'", '"')), d.objText.push(l);
                if (0 == f.pagetype) try {
                    "true" ==
