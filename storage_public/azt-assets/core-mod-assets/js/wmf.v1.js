...
WMFConverter.prototype.log=function(str) {
	return ;
    var timestamp=new Date().toISOString();
    console.log(timestamp.substr(timestamp.length-13,13)+"--"+ str);
}
WMFConverter.prototype.downsizeCanvas=function(canvas,ratio){
	var canvas_n=document.createElement("canvas");
	canvas_n.width=Math.floor(canvas.width*ratio);canvas_n.height=Math.floor(canvas.height*ratio);
	var ctx=canvas_n.getContext("2d");
	ctx.drawImage(canvas,0,0,canvas.width,canvas.height,0,0,canvas_n.width,canvas_n.height);
	canvas=null;
	return canvas_n;
}
WMFConverter.prototype.toPNG = function(str){
	this.log("start convert");
	var arr=str.split(",");
	str=arr[arr.length-1];
	var canvas=document.createElement("canvas");
	canvas.width=480;canvas.height=320;
	this.toCanvas(str,canvas);
	this.log("render canvas "+canvas.width+":"+canvas.height+":"+canvas.getAttribute("dpi"));
	var scale=1;
	var dpi=parseInt(canvas.getAttribute("dpi"));
	var fontSize=canvas.fontSize;
	if(isNaN(dpi)) dpi=72*16;
	this.log("dpi canvas "+dpi);
	if(fontSize<25) {
		this.log("end convert");
		return canvas.toDataURL();
	}else{
		//var lastCanvas=canvas;
		while(fontSize>24){
			canvas=this.downsizeCanvas(canvas,0.5);
			fontSize=fontSize/2;
		}
		this.log("end convert");
		return canvas.toDataURL();
	...
	/*this.toCanvas(str,canvas,()=>{
		log("render canvas "+canvas.width+":"+canvas.height+":"+canvas.getAttribute("dpi"));
		var scale=1;var dpi=parseInt(canvas.getAttribute("dpi"));
		if(isNaN(dpi)) dpi=72*16;
		log("dpi canvas "+dpi);
		if(dpi<150) {
			...
			//var lastCanvas=canvas;
			while(dpi>150){
				canvas=downsizeCanvas(canvas,0.5);
				dpi=dpi/2;
			}
			img.src=canvas.toDataURL();
		}
...
WMFConverter.prototype.toCanvas = function(filename, canvas, callback) {
	...
	let t = performance.now();
	try {
		parseWMF(new DataView(Base64Binary.decodeArrayBuffer(filename)), canvas);
	} catch (e) {
		console.error(e.message);
	}
	this.executeTime = performance.now() - t;
		...
		while (true) {
			...
			case RECORD_SELECT_OBJECT: {
				...
				case "FONT":
					...
					//save fontSize	
					if(obj.faceName.indexOf("Symbol")<0){
						canvas.fontSize=(Math.abs(obj.height)>canvas.fontSize) ? Math.abs(obj.height):canvas.fontSize;
					}
			...
			case RECORD_EXT_TEXT_OUT: {
				...
				//console.log("EXT_TEXT_OUT ",wh,wy,my,woy,wsy);
				//console.log("EXT_TEXT_OUT ",dv.getInt16(offset, true));
				...
				if ((options & 0x0006) > 0) {
					...
					rsize -= 4;
					//console.log("EXT_TEXT_OUT RECT ",rect);
				}
				...
				/*for(let i=0;i<count;i++){
					var dx = dv.getUint16(offset, true);
					offset += 2;
					//console.log("dx "+dx);
				}*/
				...
				if(count==1){
						...
						var dx = dv.getInt16(offset, true);offset += 2;
						//console.log("dx "+dx);
				}else{
					let fillStyle_bk = ctx.fillStyle;
					ctx.fillStyle = textColor;
					...
					for(let i=0;i<count;i++){ 
						...
						var dx = dv.getInt16(offset, true);offset += 2;
						dx=toAbsoluteX(dx,ww, wx, mx, wox, wsx);
						var textWidth=ctx.measureText(text[i]).width;
						dx=(textWidth>dx)? textWidth:dx;
						//console.log("dx "+dx);
						x+=dx;
